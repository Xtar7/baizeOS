    ...
    <div class="kb-detail-body" v-if="kbStore.current">
      ...
      <div class="kb-section">
        ...
        <div v-if="!kbStore.current.files?.length" class="empty-files">
          ...
          <p>暂无文件，上传文档开始构建知识库</p>
        </div>

        <div v-else class="file-list">
          <div
            v-for="(f, idx) in kbStore.current.files"
            :key="f.kb_file_id"
            class="file-item"
          ...
          >
            ...
            <span class="file-meta">
              {{ (f.bytes / 1024).toFixed(1) }} KB · {{ new Date(f.created_at).toLocaleDateString('zh-CN') }}
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="kb-section">
        ...
        <div class="meta-grid">
          ...
          <div class="meta-item">
            ...
            <span class="meta-value">{{ new Date(kbStore.current.created_at).toLocaleString('zh-CN') }}</span>
          </div>
        </div>
      </div>
    </div>
